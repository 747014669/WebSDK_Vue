...
let LoadUeComplete = null
let useDirect = false
function setProgress(val) {
    if (pgValue == val || val < pgValue) {
        return
    }
    pgValue = val
    var pixel_pg = document.getElementById('pixel_pg');
    if (pixel_pg) {
        if (pgtimer != null) {
            clearInterval(pgtimer)
        }
        pgtimer = setInterval(function () {
            if (pixel_pg.value < pgValue) {
                pixel_pg.value++;
                if (pixel_pg.value >= 100) {
                    document.getElementById('pixel_pg_parent').remove();
                    pixel_pg = 0
                }
            }
            else {
...
function showAfkOverlay() {
    ...
    afk.countdownTimer = setInterval(function () {
        ...
        if (afk.countdown == 0) {
            ...
            console.log('close by afk.countdown')
        } else {
...
function registerTouchEvents(playerElement) {
    ...
    console.log('inputOptions.fakeMouseWithTouches:', inputOptions.fakeMouseWithTouches)
...
function connect(usehttps) {
    ...
    ws.onmessage = function (event) {
        ...
        if (msg.type === 'config') {
            ...
            setTimeout(function () {
                if (ws) {
                    ws.send(JSON.stringify({ type: 'instance', 'to_server': serverURL }))
                }
            }, 5000)
        } else {
    ...
    ws.onclose = function (event) {
        ...
        if (ws && ws.attemptStreamReconnection === true) {
    ...
    if(!useDirect){
        ws.onopen = function (event) {
            ws.send(JSON.stringify({ type: 'instance' }))
        }
    }
}
...
function restartStream() {
    ...
    console.log('close by restartStream')
}
...
function loadUE(url, isDirect, usehttps, callback) {
    serverURL = url
    LoadUeComplete = callback;
    useDirect = isDirect;
    setProgress(90)
    setupHtmlEvents()
    registerMessageHandlers();
...
function closeUE() {
    console.log('close by player')
    pgValue = 0
    pgtimer = null
    if (ws) {
        ws.close()
        ws.onclose({ code: 0, reason: 'close' })
    }
}